...
function postCategoria(){
    if ($("#name").val().length==0 || $("#description").val().length==0 ){
        ...
        $.ajax({
            url:"http://129.151.123.186:8080/api/Category/save",
            type:"POST",
            datatype:"JSON",
...
function putCategoria(idBotonActualizar){
    ...
    $.ajax({
        url:"http://129.151.123.186:8080/api/Category/update",
        type:"PUT",
        datatype:"JSON",
...
function deleteCategoria(idBotonBorrar){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url:"http://129.151.123.186:8080/api/Category/"+ idBotonBorrar,
                type:"DELETE",
                datatype:"JSON",
             ...
             Swal.fire(
                 'Su categoria se elimino con exito.',
                 'completado'
          )
        }
      })
